...
- `reset-bootloader` needs a bootloader that implements opcode 9. Older bootloaders answer it with RESP_NG, and since the command is not expected to answer at all, the tool cannot tell the difference: it will report success while the device does nothing.
